...
class LLMPredictor(object):
    def __init__(self, llm_model_name_and_path, device='cuda', **kwargs):
        self.tokenizer = AutoTokenizer.from_pretrained(
            llm_model_name_and_path,
            use_fast=True,
            padding_side="right",
            trust_remote_code=True,
        )

        config = AutoConfig.from_pretrained(llm_model_name_and_path, trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained(llm_model_name_and_path, config=config, device_map='cuda',
        ...
        self.kwargs = kwargs
        self.device = device
        self.model_path = llm_model_name_and_path
        print("Successfully load LLM: ", llm_model_name_and_path)
    ...
    def repair_answer(self, context, query, origin_answer):
        input_ids = self.tokenizer(context, return_tensors='pt', add_special_token=False).inputs_ids
        if len(input_ids) > self.max_token:
            context = self.tokenizer.decode(input_ids[:self.max_token-1])
            warnings.warn("Texts have been truncted!")
